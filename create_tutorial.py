...
doc.add_paragraph()
add_para('구성: 총 6회차 (회차당 30분) | 총 소요 시간: 약 3시간', size=12, align=WD_ALIGN_PARAGRAPH.CENTER, bold=True)
...
# ██  1회차: 터미널과 친해지기
# ════════════════════════════════════════════════════════════
add_heading_styled('1회차: 터미널과 친해지기', level=1)
add_para('소요 시간: 30분', bold=True, color=(0, 100, 200))
add_para('이 회차를 마치면: 까만 화면(터미널)이 더 이상 무섭지 않고, 컴퓨터에게 간단한 명령을 글자로 내릴 수 있어요!', bold=True)
doc.add_paragraph()
...
add_para('다음 회차에서 프로그래밍 도구를 설치하고 AI 비서를 만나볼 거예요.', italic=True)
doc.add_page_break()

# ════════════════════════════════════════════════════════════
# ██  2회차: 프로그래밍 도구 설치
# ════════════════════════════════════════════════════════════
add_heading_styled('2회차: 프로그래밍 도구 설치 - Node.js와 Python', level=1)
add_para('소요 시간: 30분', bold=True, color=(0, 100, 200))
add_para('이 회차를 마치면: Claude Code를 설치하기 위한 준비가 완료! node, npm, python 명령어가 터미널에서 작동해요.', bold=True)
doc.add_paragraph()

add_heading_styled('Step 1. Node.js 다운로드 및 설치 (10분)', level=2)
...
doc.add_paragraph('Claude Code를 돌리려면 Node.js가 필요해요.')
doc.add_paragraph()
...
doc.add_paragraph('>> 항상 이걸 선택하면 안전해요!')
doc.add_paragraph()

doc.add_paragraph('다운로드된 파일(node-v숫자-x64.msi)을 더블클릭하세요.')
doc.add_paragraph('설치 화면이 나오면:')
doc.add_paragraph('  1) "Next" 버튼을 계속 누르세요 (4~5번)')
doc.add_paragraph('  2) "I accept the terms..." 체크박스 보이면 체크하고 "Next"')
doc.add_paragraph('  3) 마지막에 "Install" 버튼 클릭')
doc.add_paragraph('  4) "이 앱이 변경하도록 허용합니다" -> "예"')
doc.add_paragraph('  5) "Finish" 클릭')
doc.add_paragraph()
doc.add_paragraph('TIP: "Next -> Next -> Install -> Finish" 이 순서만 기억하면 돼요!')
doc.add_paragraph()

add_heading_styled('Step 2. Node.js 설치 확인 (5분)', level=2)
doc.add_paragraph('!! 중요: Git Bash를 껐다가 다시 열어야 합니다!')
doc.add_paragraph('(새로 설치한 프로그램을 인식하려면 터미널을 새로 열어야 해요)')
doc.add_paragraph()
...
doc.add_paragraph('숫자가 나오면 성공!')
doc.add_paragraph()

add_heading_styled('Step 3. Python 다운로드 및 설치 (10분)', level=2)
...
add_heading_styled('Step 4. Python 설치 확인 (5분)', level=2)
...
doc.add_paragraph('pip도 확인해볼까요?')
add_code_block('pip --version')
doc.add_paragraph('>> pip = Python의 앱스토어. Python 프로그램에 필요한 재료를 설치하는 도구')
doc.add_paragraph('숫자가 나오면 성공!')
doc.add_paragraph()
...
add_checkpoint([
    ...
    'CHECK: pip --version 결과에 숫자가 나왔나요?',
])
doc.add_paragraph()

add_heading_styled('!! 막혔을 때', level=2)
add_error_table([
    ('"node" 인식할 수 없는 명령', 'Git Bash를 새로 안 열었어요', 'Git Bash를 완전히 닫고 새로 여세요. 그래도 안 되면 Node.js 다시 설치'),
    ...
    ('어떤 버전을 받아야 할지 모르겠어요', '여러 버전이 보여서 헷갈림', '항상 "LTS" 또는 가장 위에 있는 큰 버튼을 누르세요'),
])

add_para('2회차 완료! 수고하셨습니다!', bold=True, size=12, color=(0, 128, 0))
add_para('다음 회차에서 드디어 AI 비서 Claude Code를 설치해요!', italic=True)
doc.add_page_break()

# ════════════════════════════════════════════════════════════
# ██  3회차: Claude Code 설치 및 첫 실행
# ════════════════════════════════════════════════════════════
add_heading_styled('3회차: Claude Code 설치 및 첫 실행 - AI 비서를 만나다!', level=1)
add_para('소요 시간: 30분', bold=True, color=(0, 100, 200))
add_para('이 회차를 마치면: AI 비서(Claude Code)와 터미널에서 대화할 수 있어요! "이거 만들어줘"라고 부탁할 준비 완료!', bold=True)
doc.add_paragraph()

add_heading_styled('Step 1. Claude Code 설치 (5분)', level=2)
...
add_heading_styled('Step 3. Anthropic 계정 만들기/로그인 (10분)', level=2)
doc.add_paragraph('브라우저에 로그인 화면이 나오면:')
doc.add_paragraph()
doc.add_paragraph('계정이 없다면 - 새로 가입하기:')
doc.add_paragraph('  1) "Sign up" 또는 "가입하기" 클릭')
doc.add_paragraph('  2) 이메일 주소 입력')
doc.add_paragraph('  3) 구글 계정으로도 가입 가능 - "Continue with Google" 클릭하면 더 간편!')
doc.add_paragraph('  4) 이메일로 온 인증 코드 입력')
doc.add_paragraph()
doc.add_paragraph('계정이 있다면 - 로그인:')
doc.add_paragraph('  이메일과 비밀번호 입력 후 로그인')
doc.add_paragraph()
...
add_heading_styled('Step 4. Claude에게 첫 인사 + 간단한 부탁하기 (5분)', level=2)
...
doc.add_paragraph('한 가지 더 시도해볼까요? 아래처럼 입력해보세요:')
add_code_block('간단한 인사말을 출력하는 Python 파일을 만들어줘')
doc.add_paragraph('Claude가 "파일을 만들어도 될까요?" 같은 질문을 하면')
doc.add_paragraph('"y"를 입력하고 Enter를 누르세요 (y = yes = 네)')
doc.add_paragraph()
doc.add_paragraph('Claude가 파일을 만들어주면, 아래 명령어로 확인할 수 있어요:')
add_code_block('/exit\nls')
doc.add_paragraph('새로 만들어진 .py 파일이 보일 거예요!')
doc.add_paragraph()
...
add_checkpoint([
    ...
    'CHECK: Claude에게 한국어 메시지를 보내고 답변을 받았나요?',
    'CHECK: Claude에게 파일 만들기를 부탁하고 성공했나요?',
])
doc.add_paragraph()
...
add_error_table([
    ...
    ('무료 사용량 초과 메시지', 'Claude 무료 한도 도달', '잠시 후 다시 시도하거나, Pro 구독 고려'),
])

add_para('3회차 완료! 수고하셨습니다!', bold=True, size=12, color=(0, 128, 0))
add_para('다음 회차에서 유튜브 API 열쇠를 받고 데이터 수집을 시작해요!', italic=True)
doc.add_page_break()

# ════════════════════════════════════════════════════════════
# ██  4회차: YouTube API 키 발급 + 데이터 수집
# ════════════════════════════════════════════════════════════
add_heading_styled('4회차: YouTube API 키 발급 + 데이터 수집 시작', level=1)
add_para('소요 시간: 30분', bold=True, color=(0, 100, 200))
add_para('이 회차를 마치면: 유튜브에서 ETF 관련 영상 데이터를 자동으로 가져올 수 있는 열쇠(API 키)를 갖게 되고, 데이터 수집 프로그램이 만들어져요!', bold=True)
doc.add_paragraph()

add_heading_styled('Step 1. Google Cloud Console 접속 (3분)', level=2)
...
add_heading_styled('Step 2. 새 프로젝트 만들기 (3분)', level=2)
...
add_heading_styled('Step 3. YouTube Data API 활성화 (3분)', level=2)
...
add_heading_styled('Step 4. API 키 만들기 (3분)', level=2)
...
add_heading_styled('Step 5. API 키를 프로젝트에 저장 (3분)', level=2)
...
add_heading_styled('Step 6. Claude에게 데이터 수집 프로그램 만들어달라고 부탁 (10분)', level=2)
...
add_para('4회차 완료! 수고하셨습니다!', bold=True, size=12, color=(0, 128, 0))
add_para('다음 회차에서 프로그램을 실행해서 진짜 데이터를 모아올 거예요!', italic=True)
doc.add_page_break()

# ════════════════════════════════════════════════════════════
# ██  5회차: 데이터 수집 실행 + 대시보드 제작
# ════════════════════════════════════════════════════════════
add_heading_styled('5회차: 데이터 수집 실행 + 대시보드 제작', level=1)
add_para('소요 시간: 30분', bold=True, color=(0, 100, 200))
...
add_para('5회차 완료! 핵심 프로젝트가 완성됐어요!', bold=True, size=12, color=(0, 128, 0))
...
# ██  6회차: GitHub에 공유하기
# ════════════════════════════════════════════════════════════
add_heading_styled('6회차: GitHub에 내 프로젝트 공유하기', level=1)
add_para('소요 시간: 30분', bold=True, color=(0, 100, 200))
...
doc.add_paragraph()
doc.add_paragraph()
add_para('6회차 & 전체 강의 완료! 축하합니다!!!', bold=True, size=16, color=(0, 128, 0))
...
doc.add_paragraph()
add_para('6회차 강의 전체 흐름을 한눈에 확인하세요!', italic=True, size=12)
doc.add_paragraph()
...
doc.add_paragraph()
add_para('총 소요 시간: 6회차 x 30분 = 약 3시간', bold=True, size=13)
doc.add_paragraph()
...
add_para(
    '[1회차] 터미널 설치 + Git 설치\n'
    '         |\n'
    '[2회차] Node.js + Python 설치\n'
    '         |\n'
    '[3회차] Claude Code 설치 + AI와 첫 대화\n'
    '         |\n'
    '[4회차] YouTube API 키 발급 + 수집 프로그램 생성\n'
    '         |\n'
    '[5회차] 데이터 수집 실행 + 대시보드 제작 + 브라우저 확인!\n'
    '         |\n'
    '[6회차] GitHub 가입 + 프로젝트 공유 -> 완성!',
    size=11
)
...
doc.add_paragraph()
doc.add_paragraph('축하합니다! 6회에 걸쳐 여러분은:')
